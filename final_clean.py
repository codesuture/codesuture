...
def is_keep_comment(text, is_inline):
    ...
    if is_inline:
        if 'never crash' in lower_text or 'disconnected' in lower_text or 'optional' in lower_text:
            return True

    whitelist = [
        'cpython', 'pep ', 'bytecode', 'exception table', 'ctypes', 
        'toctou', 'lock', 'security', 'compat', 'trade-off', 'tradeoff', 
        'collision', 'because', 'due to', 'prevent', 'avoid', 'force', 
        'require', 'idempotent', 'mutate', 'replay', 'de-specialize', 
        'specializ', 'cache', 'corrupt', 'fallback', 'side effect', 
        'unsafe', 'safe', 'assumption', 'cost', 'expensive', 'cheap', 
        'bottleneck', 'workaround', 'hack', 'deadlock', 'gil ', 're-entrant', 
        'reentrant', 'atomic', 'volatile', 'pointer', 'sys.monitoring', 
        'sys.settrace'
    ]
    
    blacklist = [
        'walk to the', 'find the function', 'now run', 'patching __main__',
        'jump opcodes', 'opcode name sets', 'all opcode names that',
        'dict ', 'use safe', 'check unhashable', 'print warning for',
        'trace callback', 'modules whose frames should'
    ]
...
for root, dirs, files in os.walk('.'):
    ...
    for file in files:
        if file.endswith('.py'):
            ...
            if removed:
                report[filepath] = removed

with open('comment_cleanup_report.json', 'w', encoding='utf-8') as f:
